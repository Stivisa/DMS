  ...
  try {
    ...
    if (!setting) {
      return res
        .status(404)
        .json({
          error: "Podešavanje nije pronađeno " + req.params.name,
          code: "NOT_FOUND",
        });
    }
  ...
  try {
    ...
    if (!setting) {
      return res
        .status(404)
        .json({
          error: "Podešavanje koje menjate, broj saglasnosti nije pronađeno.",
          code: "NOT_FOUND",
        });
    }
    res.status(200).json(setting);
...
//GET disk free space
router.get("/storage/free", verifyTokenAndUser, async (req, res) => {
  try {
    ...
    if (err instanceof InvalidPathError) {
      return res
        .status(400)
        .json({
          error: "Putanja diska nije pronadjena!",
          code: "DISK_PATH_NOT_FOUND",
        });
    } else {
