...
router.delete("/:id", verifyTokenAndSuperAdmin, async (req, res) => {
    try {
      ...
      if (!isEmpty) {
        return res.status(400).json({ error: "Postoje dokumenti za ovu kompaniju. Brisanje nije moguce!", code: "LINKED_DOCUMENT" });
      }
...
router.get("/:id", verifyToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Greška pri traženju kompanije.", code: "GENERIC_ERROR" });
  }
});
