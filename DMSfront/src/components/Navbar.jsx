  ...
  useEffect(() => {
    ...
    <div className="bg-white w-full border shadow h-16 px-4 flex justify-between items-center ">
            ...
            <div className="flex items-center">
                {companyOnlyOne ? (
                    <h1 className="text-4xl text-color hover:bg-gray-200 rounded-full p-2">
        ...
        <div>
            <div className="flex items-center">
                ...
                {diskError && (
                    <div
                    className="text-rose-600 mr-2 min-w-28 flex flex-col items-center justify-center"
                    >
                    {diskError}
                    </div>
                )}
                ...
                <button
                    className="button-basic flex text-white items-center"
                    onClick={() => setDropdown(!dropdown)}
                >
                    <BsPerson  size={20} className="mr-1" />
                    <span className="hidden sm:flex font-semibold">{user?.username}</span>
                </button>
                {dropdown && (
                    <div
                    onClick={() => setDropdown(!dropdown)}
            ...
            {dropdown && (
                <div className="z-10 absolute right-0 mt-2 mr-2 py-1 w-40 bg-gray-200 rounded-lg  shadow-md text-color">
                    {!companyOnlyOne && (
                    <Link to="/companiesmenu" onClick={() => { onResetActiveLink(); setDropdown(!dropdown); }}>
                         <div className="flex items-center  px-4 py-2 hover:bg-teal-400 rounded-full">
                            <BsBuildingGear size={20} className="text-teal-800 mr-1"/>
                    ...
                    <Link to="/password" onClick={() => setDropdown(!dropdown)} >
                        <div className="flex items-center  px-4 py-2 hover:bg-teal-400 rounded-full">
                            <RiLockPasswordLine  size={20} className="text-teal-800 mr-1"/>
                            <span>Promeni šifru</span>
                        </div>
                    </Link>
                    <Link to="/login" onClick={Logout}>
                        <div className="flex items-center  px-4 py-2 hover:bg-teal-400 rounded-full">
                            <TbLogout size={20} className="text-teal-800 mr-1"/>
                            <span>Odjavi se</span>
                        </div>
                    </Link>
                </div>
